			...
			<!--ここまでテンプレ -->	
				<div class="content">
						...
						<?php
							...
							if(!isset($_GET["code"]) ){
								$code="0";
								die("error1:アカウントが選択されていないです。");							
							}else{
								$code = $_GET["code"];		//アドレスに記載されているcodeをcodeに代入
							}
							
							//ここから取消行動
							$sql = "UPDATE rma_member SET del_flag=1 WHERE member_code= '$code' ";
							$res = mysql_query( $sql ) or die("失敗");
							if (!$res)die("削除失敗しました。<br />（原因）:".mysql_error());
							//完了報告
							print("取消完了しました。<br>");
